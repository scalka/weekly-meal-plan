    ...
    <Draggable draggableId={cardItem.id} index={index}>
      {(provided, snapshot) => (
        ...
        >
          ...
          <div> {cardItem.tags.map((tag: string, i: number) => <div key={i} className='inline-block bg-gray-200 rounded-full px-2 py-1 text-xs font-semibold text-gray-700 mr-1 mb-1'>{tag}</div>)}</div>
        </div>
      )}
    </Draggable>
  );
